...
export function formatAnalysisForIPFS(
  ...
  try {
    ...
    // Format weather data based on source (Open-Meteo format)
    ...
    // Format news data (GDELT format)
    const formattedNews = contextualData?.news && Array.isArray(contextualData.news) ? {
      relevantArticles: contextualData.news.slice(0, 10).map((article: any) => ({
        title: article.title || 'Untitled',
        url: article.url,
        source: article.domain || 'Unknown',
        summary: article.seendate ? `Published: ${article.seendate}` : 'No summary available'
      }))
    } : undefined;
